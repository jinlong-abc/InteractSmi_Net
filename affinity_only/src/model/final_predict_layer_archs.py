...
class DiffAbsConcatMLP(nn.Module):
    ...
    def forward(self, cf, pf):
        x = torch.cat([cf, pf, torch.abs(cf - pf), cf * pf], dim=1)  # [B, 4D]
        return self.fc(x)
...
class BilinearOuterProduct(nn.Module):
    ...
    def forward(self, cf, pf):
        b = cf.size(0)
        outer = torch.matmul(cf.view(b, -1, 1), pf.view(b, 1, -1))  # [B, D, D]
        x = outer.view(b, -1)  # flatten
        return self.fc(x)
...
class BilinearProjection(nn.Module):
    ...
    def __init__(self, d):
        ...
        self.fc_out = nn.Linear(1, 1)  # 保持接口一致
    def forward(self, cf, pf):
        score = torch.sum(cf @ self.W * pf, dim=1, keepdim=True)  # [B,1]
...
class GatedFusion(nn.Module):
    ...
    def forward(self, cf, pf):
        z = torch.cat([cf, pf], dim=1)  # [B, 2D]
        g = torch.sigmoid(self.gate(z)) # [B, D]
        fused = g * cf + (1 - g) * pf   # [B, D]
...
class EnsembleFusion(nn.Module):
    ...
    def __init__(self, d, hidden_dim=256):
        super().__init__()
        input_dim = 2*d + d + d  # concat + hadamard + diff
        self.fc = nn.Sequential(
...
# =====使用方法=======
# from fusion_methods import ConcatMLP, HadamardMLP, DiffAbsConcatMLP, BilinearOuterProduct, BilinearProjection, GatedFusion, EnsembleFusion

# class MyModel(nn.Module):
#     def __init__(self, d):
#         super().__init__()
#         # 切换不同的融合策略
#         self.fusion_head = BilinearOuterProduct(d, hidden_dim=256)

#     def forward(self, cf_final, pf_final):
#         return self.fusion_head(cf_final, pf_final)
