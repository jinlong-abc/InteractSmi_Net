...
class CoAttentionLayer(nn.Module):
    ...
    def forward(self, comp_feat: torch.Tensor, prot_feat: torch.Tensor,
        ...
        计算协同注意力
        Returns:
            comp_attended, prot_attended, comp_attention, prot_attention
        """
        # 计算亲和度矩阵
        comp_trans = torch.tanh(self.W_comp(comp_feat))  # [batch, comp_len, d_model]
        prot_trans = torch.tanh(self.W_prot(prot_feat))  # [batch, prot_len, d_model]
        
        affinity = torch.matmul(comp_trans, prot_trans.transpose(1, 2))  # [batch, comp_len, prot_len]
        ...
        # 计算双向注意力
        comp_attention = F.softmax(affinity, dim=-1)  # 化合物对蛋白质的注意力
        prot_attention = F.softmax(affinity.transpose(1, 2), dim=-1)  # 蛋白质对化合物的注意力
...
class MultiModalFusion(nn.Module):
    ...
    def forward(self, comp_feat: torch.Tensor, comp_mask: torch.Tensor,
        ...
        前向传播
        Args:
            comp_feat: 化合物特征 [batch, comp_len, latent_dim]
            comp_mask: 化合物掩码 [batch, comp_len]
            prot_feat: 蛋白质特征 [batch, prot_len, latent_dim]
            prot_mask: 蛋白质掩码 [batch, prot_len]
        Returns:
    ...
    def _bidirectional_fusion(self, comp_feat, comp_mask, prot_feat, prot_mask):
        """双向注意力融合"""
        comp_final, prot_final = self.bidirectional_module(comp_feat, comp_mask, prot_feat, prot_mask)
